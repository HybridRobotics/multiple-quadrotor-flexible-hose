function [dx] = afh_dynamics(data, x, u)
    ...
    j = 1;
    fArray = zeros(1, data.nvert);
    MomentsArray = zeros(3, data.nvert);
    Re3Array = zeros(3,data.nvert);
    
    for i = 1:data.nvert
        if data.I(i)
            fArray(i) = f(j);   
            MomentsArray(:,i) = moments(:,j);
            Re3Array(:,i) = R(:,:,j)*e3;
            j = j+1;
        else
            fArray(i) = 0;   
            MomentsArray(:,i) = zeros(3,1);
            Re3Array(:,i) = zeros(3,1);            
        end
    end
    ...
    % -------------------
    uk = data.mge3+(repmat(fArray,3,1).*Re3Array);
